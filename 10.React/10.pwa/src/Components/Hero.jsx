...
export default function Hero() {
  ...
  return (
    <div className={`mb-6 ${darkMode ? "bg-slate-900" : ""}`}>
      ...
      >
        <div
          className={`flex lg:w-[30%]  xl:w-[19%] lg:border-r ${darkMode ? "!border-r-0" : ""} border-r-gray-300 order-2 lg:order-none mx-4 md:mx-16 lg:mx-0`}
        >
          <ul className="flex flex-wrap lg:flex-col lg:gap-y-[17px] text-start text-sm sm:text-base mt-6 md:mt-9 gap-4 md:gap-6 ">
            {categories.map((items, i) => (
              <li key={i}>
                <a
                  className={` link_hover ${darkMode ? "!text-white !link_hover" : ""}`}
                  href=""
                >
                  {items}
                </a>
